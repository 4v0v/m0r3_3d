...
function Anim9:find_index(bone_name)
...
--- Add animation to Anim9 object
-- @param animation Animation data
-- @param frames Frame data
function Anim9:add_animation(animation, frames, markers)
...
function Anim9:new_track(name, weight, rate, callback, lock, early)
...
function Anim9:update(dt)
	...
	for _, track in ipairs(self.timeline) do
		...
		-- make sure f2 doesn't exceed Anim9 length or wrongly loop
		if _anim.loop then
			f2 = f2 % _anim.length
		else
...
function Anim9:reset(clear_locked)
	self.time = 0
	self.transitioning = nil
	for i = #self.timeline, 1, -1 do
...
function Anim9:transition(track, length)
	assert(track)
...
function Anim9:find_track(track)
